...
public class StatementPdfService {
    ...
    private static class SimplePdf {
        ...
        private PageRenderResult renderPage(int startRow, List<String[]> printableRows, boolean hasLogo) {
            ...
            drawRect(c, 0, 742, 595, 100, "0.02 0.22 0.14");
            drawRect(c, 0, 736, 595, 6, "0.78 0.55 0.04");

            if (hasLogo) {
                c.append("q 76 0 0 76 260 760 cm /Logo Do Q\n");
            }

            setFill(c, "1 1 1");
            text(c, "F2", 16, 48, 794, "Kimwanyi SACCO");
            text(c, "F1", 9, 48, 779, "Save - Grow - Prosper");
            text(c, "F1", 9, 48, 764, "Member Account Statement");

            setFill(c, "0.02 0.22 0.14");
            text(c, "F2", 15, 48, 706, title);
            line(c, 48, 696, 548, 696, "0.78 0.55 0.04");

            y = 682;
            setFill(c, "0.02 0.22 0.14");
            text(c, "F2", 10, 48, y, "Member Details");
            y -= 18;
            setFill(c, "0.05 0.07 0.10");
            ...
            y -= 18;
            setFill(c, "0.02 0.22 0.14");
            text(c, "F2", 10, 48, y, "Transaction Activity");
            y -= 24;
            drawRect(c, 48, y - 4, 500, 20, "0.02 0.22 0.14");
        ...
        private static void setFill(StringBuilder c, String rgb) {
    ...
    private static class PdfDocument {
        ...
        int addRawObject(String raw) {
            objects.add(raw.getBytes(StandardCharsets.ISO_8859_1));
            return objects.size();
        }
        ...
        byte[] finish() throws IOException {
            ...
            objects.add(("<< /Type /Pages /Kids [" + kids + "] /Count " + pageObjects.size() + " >>").getBytes(StandardCharsets.ISO_8859_1));
            int catalogObject = objects.size() + 1;
            objects.add(("<< /Type /Catalog /Pages " + pagesObjectNumber + " 0 R >>").getBytes(StandardCharsets.ISO_8859_1));
            ...
            for (int i = 0; i < objects.size(); i++) {
                ...
                String object = new String(objects.get(i), StandardCharsets.ISO_8859_1).replace("PAGES_REF", pagesObjectNumber + " 0 R");
                pdf.write(((i + 1) + " 0 obj\n").getBytes(StandardCharsets.ISO_8859_1));
                pdf.write(object.getBytes(StandardCharsets.ISO_8859_1));
                pdf.write("\nendobj\n".getBytes(StandardCharsets.ISO_8859_1));
            }
